...
# %%
_, embedded_sample, _, labs_sample = grouped_train_test_split(embedded, labs, labs, test_size=200)
...
def get_all_pos_in_neg(df_sample, df):
    ...
    for index, query in df_sample.iterrows():
        ...
        if isinstance(pos_with_self, pd.Series):
            continue
...
df_split = np.array_split(df_sample, num_partitions)
pool = multiprocessing.Pool(num_cores)

pos_in_negs = np.concatenate(pool.map(partial(get_all_pos_in_neg, df=df), df_split))
